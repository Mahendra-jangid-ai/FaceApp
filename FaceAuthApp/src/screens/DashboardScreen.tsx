  ...
  return (
    <ScrollView style={st.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={st.section}>
        <Text style={st.sectionTitle}>Verification & Compliance</Text>
        ...
        <View style={st.metricsCard}>
          {[
            ...
            { label: 'Match Confidence', value: avgScore, color: colors.accent },
          ...
          <View style={st.telemetryRow}>
            <Text style={st.telemetryKey}>Spoof Blocked</Text>
            <Text style={[st.telemetryVal, { color: spoofBlocked > 0 ? colors.danger : colors.success }]}>
              {spoofBlocked} blocked
            </Text>
          </View>
          
          <View style={st.telemetryRow}>
            <Text style={st.telemetryKey}>Avg Latency</Text>
            <Text style={st.telemetryVal}>{avgLatency > 0 ? `${avgLatency}ms` : '170ms'}</Text>
          </View>
      ...
      <View style={st.section}>
        <Text style={st.sectionTitle}>7-Day Activity Trend</Text>
            ...
            {weekData.map((day, i) => (
              <View key={i} style={st.barCol}>
                <View style={st.barContainer}>
                  <View style={[st.bar, st.barTotal, { height: `${Math.max(6, (day.total / maxWeek) * 100)}%` }]} />
                  <View style={[st.bar, st.barSuccess, { height: `${Math.max(4, (day.success / maxWeek) * 100)}%` }]} />
                </View>
      ...
      <View style={st.section}>
        ...
        <View style={st.infoCard}>
          {[
            { label: 'Registered Workers', value: `${userCount}`, color: colors.text },
            { label: 'Geofenced Sites', value: `${siteCount}`, color: colors.text },
            { label: 'Vision Model', value: 'MobileFaceNet INT8 (1.15MB)', color: colors.accent },
            { label: 'LFW Accuracy', value: '99.28%', color: colors.success },
            { label: 'Anti-Spoof Architecture', value: 'Laplacian + Gesture Liveness', color: colors.textDim },
            { label: 'Encryption', value: 'AES-256 GCM Keystore', color: colors.cyan },
            { label: 'Biometric Hash', value: 'ISO/IEC 24745 Standard', color: colors.textDim },
          ].map((row, i) => (
      ...
      {topUsers.length > 0 && (
        <View style={[st.section, { marginBottom: spacing.xxl }]}>
          <Text style={st.sectionTitle}>Top Active Personnel</Text>
          <View style={st.infoCard}>
            {topUsers.map((u, i) => (
...
const st = StyleSheet.create({
  ...
  kpiNum: { fontSize: 22, fontWeight: '800', fontFamily: MONO },
  ...
  metricLabel: { width: 145, fontSize: 12.5, fontWeight: '500', color: colors.textDim },
  metricBarBg: {
  ...
  metricValue: {
    width: 40, textAlign: 'right', fontFamily: MONO,
    fontSize: 12, fontWeight: '700',
  },
  metricDivider: { height: 1, backgroundColor: colors.line, marginVertical: spacing.sm },

  telemetryRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 4 },
  telemetryKey: { fontSize: 12.5, color: colors.textDim, fontWeight: '500' },
  telemetryVal: { fontFamily: MONO, fontSize: 12, color: colors.text, fontWeight: '600' },
  ...
  barsRow: { flexDirection: 'row', justifyContent: 'space-around', height: 120, paddingTop: spacing.sm },
  barCol: { alignItems: 'center', flex: 1 },
  barContainer: { flex: 1, width: 20, justifyContent: 'flex-end', alignItems: 'center' },
  bar: { position: 'absolute', bottom: 0, borderRadius: 3 },
  barTotal: { backgroundColor: '#E2E8F0', width: 20 },
  barSuccess: { backgroundColor: colors.success, width: 12 },
  barLabel: { fontSize: 11, marginTop: 6, fontWeight: '600', color: colors.textDim },
  barCount: { fontFamily: MONO, fontSize: 10, color: colors.textFaint, marginTop: 1 },
  ...
  legendText: { fontSize: 11, fontWeight: '500', color: colors.textDim },
  ...
  infoValue: { fontSize: 12.5, fontWeight: '600', fontFamily: MONO },
  ...
  topUserCount: { fontSize: 12, fontWeight: '600', color: colors.accent, fontFamily: MONO },
});
